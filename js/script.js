...
function drawUI() {    
    ...
    ctx.textAlign = 'left';
    ctx.fillText('Swaptris',0.9,3.4);

    ctx.font = '1px monospace';
    ctx.fillText('by Cloud Worm',1,5);

    ctx.fillText('← / → = Move Horizontally',6,24);
...
function lineCheck() {
    ...
    for(var y = arena.matrix.length - 1; y > 0; y--) {
        if(arena.matrix[y].every(function(x) {return x > 0;})) {
            ...
            // Level (changes every 10 lines cleared)
            level = Math.floor(linesCleared/2);
        }
    }
}
